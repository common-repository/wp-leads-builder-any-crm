...
class WPCapture_includes_helper_PRO {
	...
	public static function deactivate(){
		...
		global $IncludedPluginsPRO;
		global $wpdb;
		$droptablesettings = get_option('wp_droptable_settings');
		...
		$table_array = ['wp_smackleadbulider_shortcode_manager','wp_smack_ecom_info','wp_smackleadbulider_field_manager','wp_smackleadbulider_form_field_manager','wp_smackthirdpartyformfieldrelation','wp_smackformrelation'];
	...
	public static function migrateFromOlderToNewerVersion()
	{
		...
		for($i=0;$i<7;$i++)
		{
			...
			if (is_array($plugin_field_value)) {
			...
			if (!empty($contact_post_id) && is_array($contact_post_id) && isset($contact_post_id[0])) {
			...
			if (!empty($contact_widget_id) && is_array($contact_widget_id) && isset($contact_widget_id[0])) {
			...
			if (!empty($short_id) && is_array($short_id) && isset($short_id[0])) {
			...
			if (is_object($short_third_relation_id) && property_exists($short_third_relation_id, 'id')) {
			...
			$short_third_relation_id = $wpdb->get_results($wpdb->prepare("select id from wp_smackthirdpartyformfieldrelation order by id DESC limit 1", array()));
			if (!empty($short_third_relation_id) && is_array($short_third_relation_id) && isset($short_third_relation_id[0])) {
			...
			if (!empty($contact_post_ids) && is_array($contact_post_ids) && isset($contact_post_ids[0])) {
			...
			if (!empty($max_rel_id_post) && is_array($max_rel_id_post) && isset($max_rel_id_post[0])) {
			...
			if (!empty($post_shortcode_id) && is_array($post_shortcode_id) && isset($post_shortcode_id[0])) {
			...
			if (!empty($widget_shortcode_id) && is_array($widget_shortcode_id) && isset($widget_shortcode_id[0])) {
			...
			if (!empty($max_rel_id_widget) && is_array($max_rel_id_widget) && isset($max_rel_id_widget[0])) {
			...
			{
				$field_id = $wpdb->get_results($wpdb->prepare("select field_id from wp_smackleadbulider_field_manager where field_label=\"{$plugin_form_post_value['fields'][$k]['label']}\" and crm_type='{$Plugins_New[$Plugins[$i]]}'", array()));
				if (!empty($field_id) && is_array($field_id) && isset($field_id[0])) {
				$field_id = $field_id[0]->field_id;
				}
